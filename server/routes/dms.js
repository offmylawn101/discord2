  ...
  try {
    ...
    for (const row of channels) {
      ...
      if (row.member_id !== req.userId) {
        const user = await db.get('SELECT id, username, discriminator, avatar, status FROM users WHERE id = ?', [row.member_id]);
...
router.post('/', authenticate, async (req, res) => {
  try {
    ...
    const recipient = await db.get('SELECT id, username, discriminator, avatar, status FROM users WHERE id = ?', [recipient_id]);
    ...
    if (existingChannel) {
      const channel = await db.get('SELECT * FROM channels WHERE id = ?', [existingChannel.channel_id]);
    ...
    await db.transaction(async (tx) => {
      ...
      `, [channelId, `dm-${req.userId}-${recipient_id}`]);

      await tx.run('INSERT INTO dm_members (channel_id, user_id) VALUES (?, ?)', [channelId, req.userId]);
      await tx.run('INSERT INTO dm_members (channel_id, user_id) VALUES (?, ?)', [channelId, recipient_id]);
    });
...
router.post('/group', authenticate, async (req, res) => {
  try {
    ...
    await db.transaction(async (tx) => {
      ...
      `, [channelId, name || 'Group DM']);

      for (const userId of allMembers) {
        await tx.run('INSERT INTO dm_members (channel_id, user_id) VALUES (?, ?)', [channelId, userId]);
      }
    });

    const channel = await db.get('SELECT * FROM channels WHERE id = ?', [channelId]);
    channel.members = await db.all(`
